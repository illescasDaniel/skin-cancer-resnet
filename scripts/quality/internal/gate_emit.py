...
def cmd_codespell(stdin: TextIO | None = None) -> int:
	src = stdin or sys.stdin
	text = src.read()
	errors = 0
	for line in text.splitlines():
		...
		if not stripped:
			continue
		errors += 1
		_emit_error(sys.stdout, stripped, title="codespell")
	_print_summary(errors, 0)
	return 1 if errors else 0
...
def main() -> int:
	if len(sys.argv) < 2:
		print("usage: gate_emit.py {pyright|ruff-github|audit|codespell}", file=sys.stderr)
		return 2
	command = sys.argv[1]
	match command:
		case "pyright":
